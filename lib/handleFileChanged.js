...
const handleFileChanged = function (config, logger, preLoads, sprocketsPreloads, postLoads, fileList) {
    ...
    return function (files) {
        ...
        // assuming no watching
        var newPatterns = _.map(preLoads, (fileName) => createPattern(fileName, false));
        fetchMetadata(sprocketsPreloads).then((metadata) => {
            ...
            // right here we need to add requires in for opal-rspec and all of our tests here
            newPatterns = newPatterns.concat(_.map(postLoads, (fileName) => createPattern(fileName, false)));
