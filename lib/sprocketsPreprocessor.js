...
const createSprocketsProcessor = function (args, config, logger) {
    ...
    return function (content, file, done) {
        ...
        if (opalConfig.fileMetadata === null) {
            console.log("no promise for "+file.originalPath);
            done('');
            return;
        }
        opalConfig.fileMetadata.then((metadata) => {
            const ourOwnMetadata = metadata[file.originalPath];
