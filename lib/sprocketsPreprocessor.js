...
function getOpalRequire(fileMetadata) {
    var parsed = path.parse(fileMetadata.logical_path)
    // extname cuts off the directory too, don't want that
    var relativePath = path.join(parsed.dir, parsed.name)
    return `Opal.require('${relativePath}');`
}

function isTest(metadata) {
    const logical_path = metadata.logical_path
    // these things don't require themselves
    // at this point, _spec.rb will have a .js suffix
    return (logical_path === 'opal-rspec.js') || (logical_path.indexOf('_spec.js') != -1)
}
...
const createSprocketsProcessor = function (args, config, logger) {
    ...
    return function (content, file, done) {
        ...
        httpGet(url).then((res) => {
            ...
            res.on('end', () => {
                if (isTest(ourOwnMetadata)) {
                    js += getOpalRequire(ourOwnMetadata)
                }
