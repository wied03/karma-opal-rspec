...
const createOpalSourceMapMWare = function (config, logger) {
    ...
    return function (request, response, next) {
        if (request.url.endsWith('.rb')) {
            const originalSource = config.sprocketsSrcMapSourcesMap[request.url];
            const newUrl = `${config.opal.rackServer.baseUrl}${originalSource}`;
            ...
            const sourceMapUrl = config.sprocketsSrcMap[request.url];
