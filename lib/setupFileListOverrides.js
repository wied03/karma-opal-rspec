...
const setupFileListOverrides = function (config, logger, preLoads, sprocketsPreloads, postLoads, fileList) {
    ...
    const getTestData = (files, pattern, updates) => {
        return fetchMetadata(files).then((newMetadata) => {
            ...
            const testDependencyMetadata = _.pickBy(newMetadata, (fileInfo, fileName) => !_.includes(files, fileName));
            ...
            var requireClauses = ["Opal.require('opal/rspec');"]
                .concat(_.map(files, (fileName) => `Opal.require('${pathLib.basename(newMetadata[fileName].logical_path, '.js')}');`))
            var requiresContents = requireClauses.join("\n")
