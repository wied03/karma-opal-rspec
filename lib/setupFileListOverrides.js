...
// TODO: For https://github.com/wied03/karma-opal-rspec/issues/20, this should be moved out of here
function generateRequiresFile(testMetadata, testRequiresFile, newPatterns) {
    // spec files with invalid dependencies will show up here but they don't have logical paths
    const validMetadata = _.pickBy(testMetadata, (fileInfo) => !fileInfo.error)
    const requireClauses = ['Opal.require(\'opal/rspec\');']
        .concat(_.map(validMetadata, (fileInfo) => {
            var parsed = pathLib.parse(fileInfo.logical_path)
            // extname cuts off the directory too, don't want that
            var relativePath = pathLib.join(parsed.dir, parsed.name)
