...
const setupFileListOverrides = function (config, logger, preLoads, sprocketsPreloads, postLoads, fileList) {
    ...
    const getTestData = (files, pattern, updates) => {
        return fetchMetadata(files).then((newMetadata) => {
            ...
            var requiresContents = "Opal.require('opal/rspec');\n"
            var requireClauses = _.map(files, (fileName) => `Opal.require('${pathLib.basename(newMetadata[fileName].logical_path, '.js')}');\n`)
            _.forEach(requireClauses, (clause) => requiresContents += clause)
