...
const setupFileListOverrides = function (config, logger, fileList, emitter) {
    const log = logger.create('fileListOverride.karma_sprockets')
    const noWatchPatterns = (filenames) => _.map(filenames, (fileName) => createPattern(fileName, false))

    return function (preLoads, sprocketsPreloads, postLoads, rackProcess) {
...
setupFileListOverrides.$inject = ['config', 'logger', 'fileList', 'emitter']

module.exports = {
    'fileListOverrider': ['factory', setupFileListOverrides]
}
