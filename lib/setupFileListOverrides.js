...
const setupFileListOverrides = function (config, logger, preLoads, sprocketsPreloads, postLoads, fileList) {
    ...
    const getTestData = (specFiles, pattern, updates) => {
        return fetchMetadata(specFiles).then((newMetadata) => {
            ...
            const requireClauses = ["Opal.require('opal/rspec');"]
                .concat(_.map(specFiles, (fileName) => `Opal.require('${pathLib.basename(newMetadata[fileName].logical_path, '.js')}');`))
    ...
    const getPatternsWithDeps = (specPatterns) => {
        ...
        return getSprocketsPreloadData(preLoadPatterns).then((preloadUpdates) => {
            return Promise.reduce(specPatterns, (updates, patternObject) => {
