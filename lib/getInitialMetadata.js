...
const waitForServer = function (config, log) {
    ...
    var request = {
        files: ['opal', 'opal-rspec', 'karma_reporter'],
        watch: false,
        exclude_self: false
    }
    ...
    while (metadataResponse === null) {
        retries += 1;
        try {
            metadataResponse = httpSync(`${opalConfig.rackServer.baseUrl}/metadata`, {
                timeout: opalConfig.rackRequestTimeout,
                method: 'POST',
                data: request,
                contentType: 'json'
            });
    ...
    return JSON.parse(metadataResponse.data.toString());
};
