...
const getTestMetadata = require('./karma_sprockets/metadataFetcher')
...
const initOpal = function (files, logger, config, fileListOverrider) {
    ...
    env.PATTERN = _.map(files, 'pattern')
    ...
    _.forOwn(metadata.roll_ups, (meta, file) => {
        files.unshift(createPattern(file, false))
    })
    const newPatterns = _map(opalConfig.loadPaths, path => {
        `${path}/**/*.{js,rb}`
    })
    _.forEach(newPatterns, pattern => {
        files.unshift(createPattern(pattern, true))
    })
    files.push(require.resolve('stacktrace-js/dist/stacktrace-with-promises-and-json-polyfills.min'))
    files.push(`${__dirname}/opal_rspec/runner.js`)
