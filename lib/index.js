...
var initOpal = function (files, preprocessors, logger, config) {
    ...
    env.PATTERN = pattern;
    var rack = spawn('bundle', ['exec', 'rackup'], {stdio: 'inherit', env: env});
    ...
    var metadata = JSON.parse(child_process.execSync("bundle exec ruby lib/get_metadata.rb " + pattern + " " + tmpobj.name).toString());
