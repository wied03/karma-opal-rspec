...
const fs = require('fs');
const waitForRackServer = require('./waitForRackServer');
...
const initOpal = function (files, logger, config) {
    ...
    var metadata = waitForRackServer(config, log);
    var patterns = _.map(metadata, function (fileInfo, filename) {
        return createPattern(filename, fileInfo.watch);
    });
    _.forEach(_.reverse(patterns), function (pattern) {
